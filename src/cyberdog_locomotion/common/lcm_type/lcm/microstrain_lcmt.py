...
class microstrain_lcmt(object):
    __slots__ = ["quat", "rpy", "rpy_imu", "omega", "acc", "temp", "good_packets", "bad_packets", "timestamp"]

    __typenames__ = ["float", "float", "float", "float", "float", "float", "int64_t", "int64_t", "int64_t"]

    __dimensions__ = [[4], [3], [3], [3], [3], None, None, None, None]

    def __init__(self):
        self.quat = [ 0.0 for dim0 in range(4) ]
        self.rpy = [ 0.0 for dim0 in range(3) ]
        self.rpy_imu = [ 0.0 for dim0 in range(3) ]
    ...
    def _encode_one(self, buf):
        ...
        buf.write(struct.pack('>3f', *self.rpy_imu[:3]))
        buf.write(struct.pack('>3f', *self.omega[:3]))
        buf.write(struct.pack('>3f', *self.acc[:3]))
        buf.write(struct.pack(">fqqq", self.temp, self.good_packets, self.bad_packets, self.timestamp))
    ...
    def _decode_one(buf):
        ...
        self.rpy_imu = struct.unpack('>3f', buf.read(12))
        self.omega = struct.unpack('>3f', buf.read(12))
        self.acc = struct.unpack('>3f', buf.read(12))
        self.temp, self.good_packets, self.bad_packets, self.timestamp = struct.unpack(">fqqq", buf.read(28))
        return self
    _decode_one = staticmethod(_decode_one)

    def _get_hash_recursive(parents):
        if microstrain_lcmt in parents: return 0
        tmphash = (0x5cd056dfbad25aeb) & 0xffffffffffffffff
